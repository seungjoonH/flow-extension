...
describe("repository", () => {
  ...
  describe("2. 모든 고정 확장자 리스트가 default 값이 unchecked 인가?", () => {
    it("초기 조회 시 고정 항목은 모두 checked === 0 인가?", () => {
      const fixed = repo.getFixedExts();
  ...
  describe("4. 동일한 커스텀 확장자 이름을 두 번 저장하면 DB 가 거부하는가?", () => {
    it("두 번째 INSERT 는 UNIQUE 제약으로 실패하는가?", () => {
  ...
  describe("5. 커스텀 확장자를 추가할 수 있는가?", () => {
    it("INSERT changes 가 1이고 조회 목록에 해당 이름이 있는가?", () => {
  ...
  describe("6. 커스텀 확장자를 최대 CUSTOM_MAX 건까지 DB 에 쌓을 수 있는가?", () => {
    it("서로 다른 이름을 CUSTOM_MAX 번 넣으면 조회 시 CUSTOM_MAX 건인가?", () => {
  ...
  describe("8. 커스텀 확장자 전체 삭제", () => {
    it("deleteAllCustomExts 후 커스텀이 비고 고정 행 개수가 유지되는가?", () => {
      const ts = Date.now();
